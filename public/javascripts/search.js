...
$('#query').keydown((e) => {
    ...
    if(e.which == 13 && $('#query').val().length != 0) {
        let tagname = $('#query').val();
        let gender = (tagname.split(':')[0] == '남' || tagname.split(':')[0] == 'male') ? 'male' : (tagname.split(':')[0] == '여' || tagname.split(':')[0] == 'female') ? 'female' : 'tag'
        let html = `<a class="tag" gender="${gender}">${tagname}</a>`;
        $(`<li>${html}</li>`).appendTo('#tags')
        $('#query').val('');
    }else if(e.which == 13 && $('#query').val().length != 0) {
        $('#sumbit').click();
    }else if(e.which == 8 && $('#query').val().length == 0) {
        console.log($('#tags > li').last())
        $('#tags > li').last().remove()
    }else{
        //$(e.).appendTo
    }
    ...
    t.fn.tagEditorInput = function() {
        ...
        return window.getSelection && t(document).off("keydown.tag-editor").on("keydown.tag-editor", function(e) {
            if (8 == e.which || 46 == e.which || e.ctrlKey && 88 == e.which) {
                try {
                    var a = getSelection(),
                        r = "BODY" == document.activeElement.tagName ? t(a.getRangeAt(0).startContainer.parentNode).closest(".tag-editor") : 0
                } catch (e) {
                    r = 0
                }
            ...
            function a(e) {
                for (var a, n = e.closest("li"), d = e.val().replace(/ +/, " ").split(o.dregex), g = e.data("old_tag"), f = c.slice(0), h = !1, u = 0; u < d.length; u++)
                    if (v = t.trim(d[u]).slice(0, o.maxLength), o.forceLowercase && (v = v.toLowerCase()), a = o.beforeTagSave(r, s, f, g, v), v = a || v, a !== !1 && v && (o.removeDuplicates && ~t.inArray(v, f) && t(".tag-editor-tag", s).each(function() {
            ...
            s.click(function(e, i) {
                ...
                return o.beforeTagDelete(r, s, c, l.text()) === !1 ? !1 : (l.addClass("deleted").animate({
                    width: 0
                ...
                if (a.ctrlKey || a.which > 1) {
                    ...
                    return o.beforeTagDelete(r, s, c, n.text()) === !1 ? !1 : (n.addClass("deleted").animate({
                        width: 0
                    }, o.animateDelete, function() {
                        l.remove(), e()
                    }), i(), !1)
